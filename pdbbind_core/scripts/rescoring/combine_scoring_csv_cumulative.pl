...
sub print_line {
    ...
    return if -e "${protein}_pocket/${top_percent}/cumulative.tbl";
    ...
    if ( $#score == -1 ) {
        `touch ${protein}_pocket/${top_percent}/cumulative.tbl`;
        return;
    }
    ...
    open my $outfile, ">", "${protein}_pocket/${top_percent}/cumulative.tbl";
