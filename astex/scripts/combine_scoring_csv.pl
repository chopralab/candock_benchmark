...
sub print_line {
    ...
    if ( $#score == -1 ) {
        `touch ${protein}$ENV{suffix}/${top_percent}/combined.tbl`;
        return;
    }
    ...
    open my $outfile, ">", "${protein}$ENV{suffix}/${top_percent}/combined.tbl";
...
my $ROOT_DIR="$ENV{RCAC_SCRATCH}/$ENV{benchmark_set}";
